  ...
  const caseStudies: CaseStudy[] = [
    {
      ...
      results: [
        { metric: "Revenue Growth", improvement: "+185%", icon: <TrendingUp className="w-5 h-5" /> },
        { metric: "System Uptime", improvement: "99.9%", icon: <Clock className="w-5 h-5" /> },
        { metric: "Customer Satisfaction", improvement: "+92%", icon: <Users className="w-5 h-5" /> }
      ]
    },
    {
      ...
      results: [
        { metric: "Security Incidents", improvement: "-100%", icon: <TrendingUp className="w-5 h-5" /> },
        { metric: "Compliance Score", improvement: "100%", icon: <Clock className="w-5 h-5" /> },
        { metric: "Cost Savings", improvement: "$3.5M/year", icon: <Users className="w-5 h-5" /> }
      ]
    },
    {
      ...
      results: [
        { metric: "Processing Speed", improvement: "+320%", icon: <TrendingUp className="w-5 h-5" /> },
        { metric: "Medical Errors", improvement: "-95%", icon: <Clock className="w-5 h-5" /> },
        { metric: "Patient Satisfaction", improvement: "+88%", icon: <Users className="w-5 h-5" /> }
      ]
    }
  ];
    ...
    <div className="mt-16">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {caseStudies.map((study, index) => (
          <Card 
            key={index}
            className="group hover:scale-105 transition-all duration-300 cursor-pointer bg-card/60 backdrop-blur-sm border-primary/20 hover:border-primary/50 hover:shadow-xl hover:shadow-primary/20 animate-fade-in"
              ...
              <CardContent className="space-y-4 animate-fade-in">
                ...
                <div className="space-y-3">
                  ...
                  {study.results.map((result, idx) => (
                    <div 
                      key={idx} 
                      className="flex items-center justify-between p-3 bg-accent/5 rounded-lg border border-accent/10 hover:bg-accent/10 transition-colors"
                    >
